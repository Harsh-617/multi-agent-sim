"""CooperativeLeagueRegistry – manages saved cooperative agent snapshots.

Mirrors LeagueRegistry exactly but stores under:
...
class CooperativeLeagueRegistry:
    """Filesystem-backed registry of cooperative league member snapshots."""

    def __init__(
        self,
        league_root: str | Path = "storage/agents/cooperative/league",
    ) -> None:
    ...
    def load_member(self, member_id: str) -> Path:
        ...
        if not member_dir.is_dir():
            raise KeyError(f"Unknown cooperative league member: {member_id!r}")
        return member_dir
